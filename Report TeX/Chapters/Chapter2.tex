...
The authors propose to approximate the average powers from Equations \ref{eq:aperiodicpower} and \ref{eq:periodicpower}.
...
While this concept is likely to be robust to various non-stationary noise types, by definition it cannot cope with the unvoiced parts of speech, detection of which is performed by a hang-over scheme.
...
\begin{equation}
\log \Lambda_v = \frac{1}{\vartheta} \sum_{k=1}^{\vartheta} \log \Lambda(k f_0)
\label{eq:harmfreq}
\end{equation}
