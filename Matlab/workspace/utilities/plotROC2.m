function plotROC2(white,car,spchspect,babble,opsroom,factory,snr)

subplot(2,3,1); plotLine(white); grid;
title(strcat('white',{' '},snr,{' '},'dB')); ylabel('True Positive Rate'); xlabel('False Positive Rate');
subplot(2,3,2); plotLine(car); grid;
title(strcat('car',{' '},snr,{' '},'dB')); ylabel('True Positive Rate'); xlabel('False Positive Rate');
subplot(2,3,3); plotLine(spchspect); grid;
title(strcat('spchspect',{' '},snr,{' '},'dB')); ylabel('True Positive Rate'); xlabel('False Positive Rate');
subplot(2,3,4); plotLine(babble); grid;
title(strcat('babble',{' '},snr,{' '},'dB')); ylabel('True Positive Rate'); xlabel('False Positive Rate');
subplot(2,3,5); plotLine(opsroom); grid;
title(strcat('opsroom',{' '},snr,{' '},'dB')); ylabel('True Positive Rate'); xlabel('False Positive Rate');
subplot(2,3,6); plotLine(factory); grid;
title(strcat('factory',{' '},snr,{' '},'dB')); ylabel('True Positive Rate'); xlabel('False Positive Rate');

legend('Sohn','LTSD','PARADE','Harmfreq');
subplot(2,3,1); plotScatter(white);
subplot(2,3,2); plotScatter(car);
subplot(2,3,3); plotScatter(spchspect);
subplot(2,3,4); plotScatter(babble);
subplot(2,3,5); plotScatter(opsroom);
subplot(2,3,6); plotScatter(factory);
